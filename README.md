Passive Network Packet Sniffer
Project Overview
This is a basic passive network packet sniffer written in Python, built as a networking internship task.
...
1.	Install Python 3 from python.org if not already installed. Verify with:
2.	python --version
3.	Install Scapy:
4.	pip install scapy
5.	(Windows only) Install Npcap:
o	Download from npcap.com/#download
o	Run the installer as Administrator
o	During setup, check the box "Install Npcap in WinPcap API-compatible Mode"
o	Restart the PC after installation
6.	Save the script as network-sniffing.py in your project folder.
________________________________________
How to Run
...
•	Do not use this on a network you don't own or don't have permission to monitor (e.g., public Wi-Fi, a neighbor’s network, an employer's network without authorization), this can be illegal.
...
•	The actual concept behind passive sniffing (reading existing traffic).
